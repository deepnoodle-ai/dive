...
func TestStreamingBasicText(t *testing.T) {
	...
	assert.True(t, hasText, "Expected to receive text content in stream")
	...
	assert.True(t, hasStart, "Expected message start event")
	assert.True(t, hasStop, "Expected message stop event")
	assert.Equal(t, "Hello, world!", textAccumulator)
}
...
func TestStreamingWithToolCall(t *testing.T) {
	...
	assert.NoError(t, stream.Err())
	assert.NotEmpty(t, events)
	assert.True(t, hasToolUse, "Expected to receive tool use in stream")
	...
	assert.True(t, strings.HasPrefix(toolUseID, "call_"))
...
func TestStreamingReasoning(t *testing.T) {
	...
	assert.NoError(t, stream.Err())
	assert.NotEmpty(t, thinkingAccum, "Expected to receive thinking content")
	// It should be "3x² + 4x − 5" but leave off a chunk of that because
	// the formatting can vary
	assert.True(t, strings.Contains(thinkingAccum, " + 4x"),
		"Expected to find derivative result, got: %s", thinkingAccum)
	assert.Contains(t, strings.ToLower(thinkingAccum), "derivative")

	assert.True(t, strings.Contains(responseAccum, " + 4x"),
		"Expected to find derivative result, got: %s", responseAccum)
	assert.Contains(t, strings.ToLower(responseAccum), "derivative")
