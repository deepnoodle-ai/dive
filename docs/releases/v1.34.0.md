...
session. `WithContinue` picks it up without Dive rerunning any tool; the model
is told which calls have unknown results and to check them before calling them
again. `WithSoftCancel` stops a run at the next step boundary.
