...
session. `WithContinue` calls the model again without replaying any recorded
tool call, and `WithSoftCancel` stops a run at the next step boundary. When a
call's result is unknown, `Outcome.Next` is `reconcile`: check whether the call
took effect before continuing, since the model may request it again.
