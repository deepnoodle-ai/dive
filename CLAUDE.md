...
- `skill/` — Unified skills and slash commands. `skill.Loader` implements `dive.Extension` — pass it to `AgentOptions.Extensions` to wire up the Skill tool, catalog hook, and content hook. Three-layer architecture: rules in system prompt, a typed contextual `<system-reminder name="skills">` pinned into a model-facing copy of the first user message, and the Skill tool as a trigger with content via PostToolUseHook. Provider-based loading (filesystem, `.agents/skills/`), variable expansion, trigger matching. New integrations use `Reminder`, `WithPinnedReminder`, and `HookContext.PinReminder`/`AppendReminder`; `SetSystemReminder` is the legacy plain-text compatibility path.
