...
- `skill/` — Unified skills and slash commands. `skill.ConfigureAgent(&opts, loader)` wires up the Skill tool, Toolset, catalog hook, and content hook. Three-layer architecture: rules in system prompt, catalog as `<system-reminder name="skills">` in first user message, tool as trigger with content via PostToolUseHook. Provider-based loading (filesystem, HTTP, `.agents/skills/`), variable expansion, allowed-tools filtering, trigger matching. `dive.SetSystemReminder` manages named blocks in conversation context.
