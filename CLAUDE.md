...
- `session/` — Persistent conversation state: `Session` struct (implements `dive.Session` and `dive.TurnStore`), `Store` interface, `MemoryStore`, `FileStore`, Fork (up to the last completed turn by default), Compact, Turns, RemoveLastTurn.
