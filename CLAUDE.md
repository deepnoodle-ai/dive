...
- `todo/` — TodoWrite tool, types (`TodoStatus`, `TodoItem`, `TodoEvent`), and `TodoTracker`. `todo.Extension` implements `dive.Extension`: provides the stateless `TodoWrite` tool, persists the latest successful write in message history, and injects a `<system-reminder name="todos">` block when the model hasn't used the tool in many turns (default 10, tunable via `WithReminderTurns`). State lives entirely in conversation history and survives compaction — safe to share one extension instance across agents and subagents.
