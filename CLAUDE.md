...
- `subagent/` — Subagent catalog: `Definition` (prompt, allowed/disallowed tools, model), built-in read-only `Explore`/`Plan` and `GeneralPurpose`, `FilterTools` (a `ToolDeclarer` counts as its declared tools), and a `Loader` (markdown + YAML frontmatter). Catalogs are plain `map[string]*Definition`; `DescribeTypes()` renders the tool description.
