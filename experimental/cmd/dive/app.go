...
import (
	...
	"github.com/deepnoodle-ai/dive/experimental/interactor"
...
func (a *App) ConfirmTool(ctx context.Context, toolName, summary string, input []byte) (bool, error) {
	// For now, just prompt without session memory
	// (Permission logic removed for simplification)

	// Parse input JSON to build better summary and preview
	var parsed map[string]interface{}
	var contentPreview string
	var actionSummary string
	toolCatLabel := "operations" // Generic fallback
	...
	a.runner.SendEvent(showDialogEvent{
		...
		dialog: &DialogState{
			...
			ConfirmToolCategory:      toolName,
			ConfirmToolCategoryLabel: toolCatLabel,
		},
	})
	...
	case result := <-confirmChan:
		...
		// (Permission manager removed for simplification)
...
func (a *App) SelectTool(ctx context.Context, req *interactor.SelectRequest) (*interactor.SelectResponse, error) {
	// Build options
	...
	case result := <-selectChan:
		...
		if result.OtherText != "" {
			// User selected "Other" and typed custom text
			return &interactor.SelectResponse{OtherText: result.OtherText}, nil
		}
		if result.Index < 0 {
			return &interactor.SelectResponse{Canceled: true}, nil
		}
		return &interactor.SelectResponse{Value: req.Options[result.Index].Value}, nil
	case <-ctx.Done():
		a.runner.SendEvent(hideDialogEvent{baseEvent: newBaseEvent()})
		return &interactor.SelectResponse{Canceled: true}, ctx.Err()
	}
}

// MultiSelectTool prompts for multiple selections
func (a *App) MultiSelectTool(ctx context.Context, req *interactor.MultiSelectRequest) (*interactor.MultiSelectResponse, error) {
	// Build options
	options := make([]DialogOption, len(req.Options))
	...
	case indices := <-multiSelectChan:
		a.runner.SendEvent(hideDialogEvent{baseEvent: newBaseEvent()})
		if indices == nil {
			return &interactor.MultiSelectResponse{Canceled: true}, nil
		}
		var values []string
		for _, idx := range indices {
			values = append(values, req.Options[idx].Value)
		}
		return &interactor.MultiSelectResponse{Values: values}, nil
	case <-ctx.Done():
		a.runner.SendEvent(hideDialogEvent{baseEvent: newBaseEvent()})
		return &interactor.MultiSelectResponse{Canceled: true}, ctx.Err()
	}
}

// InputTool prompts for text input
func (a *App) InputTool(ctx context.Context, req *interactor.InputRequest) (*interactor.InputResponse, error) {
	// Set up dialog via event
	...
	case value := <-inputChan:
		...
		if value == "" && req.Default == "" {
			return &interactor.InputResponse{Canceled: true}, nil
		}
		return &interactor.InputResponse{Value: value}, nil
	case <-ctx.Done():
		a.runner.SendEvent(hideDialogEvent{baseEvent: newBaseEvent()})
		return &interactor.InputResponse{Canceled: true}, ctx.Err()
	}
}
