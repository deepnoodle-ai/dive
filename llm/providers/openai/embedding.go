...
var _ dive.EmbeddingProvider = &EmbeddingProvider{}
...
// EmbeddingRequest represents the OpenAI embeddings API request structure.
...
// EmbeddingAPIResponse represents the OpenAI embeddings API response structure.
type EmbeddingAPIResponse struct {
	Object string                `json:"object"`
	Data   []EmbeddingAPIObject  `json:"data"`
	Model  string                `json:"model"`
	Usage  EmbeddingAPIUsage     `json:"usage"`
}

// EmbeddingAPIObject represents a single embedding object from the API.
type EmbeddingAPIObject struct {
	Object    string    `json:"object"`
	Index     int       `json:"index"`
	Embedding []float64 `json:"embedding"`
}

// EmbeddingAPIUsage represents usage information from the API.
type EmbeddingAPIUsage struct {
...
func (p *EmbeddingProvider) GenerateEmbedding(ctx context.Context, opts ...dive.EmbeddingOption) (*dive.EmbeddingResponse, error) {
	config := &dive.EmbeddingConfig{}
	config.Apply(opts)
	...
	embeddings := make([]dive.Embedding, len(result.Data))
	for i, data := range result.Data {
		embeddings[i] = dive.Embedding{
			Index:  data.Index,
	...
	response := &dive.EmbeddingResponse{
		Embeddings: embeddings,
		Model:      result.Model,
		Usage: dive.EmbeddingUsage{
