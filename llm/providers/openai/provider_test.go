...
func TestProvider_ensureSchemaAdditionalPropertiesFalse(t *testing.T) {
	provider := New()
	...
	tests := []struct {
		...
		{
			name: "schema with additionalProperties already set",
			input: schema.Schema{
				Type:                 "object",
				Required:             []string{"name"},
			...
			expected: schema.Schema{
				Type:                 "object",
				Required:             []string{"name"},
				AdditionalProperties: &[]bool{true}[0], // Should not be changed
				Properties: map[string]*schema.Property{
					"name": {
						Type:        "string",
						Description: "The name",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := provider.ensureSchemaAdditionalPropertiesFalse(tt.input)
			assert.Equal(t, tt.expected, result)
		})
	}
}
