...
func getProvider() llm.LLM {
	...
	case "openai":
		return openai.New()
	case "groq":
		return groq.New(groq.WithModel(model))
	default:
		return anthropic.New() // Default to Anthropic
	}
}

func runChatSession() error {
	...
	chatAgent, err := agent.NewAgent(agent.AgentOptions{
		Name: "Assistant",
		Backstory: `You are a helpful AI assistant. You aim to be direct, clear, and helpful in your responses.
You can engage in general conversation and help with various tasks.`,
		LLM:          llmProvider,
...
var chatCmd = &cobra.Command{
	Use:   "chat",
	Short: "Start an interactive chat session with an AI agent",
	Long:  `Start an interactive chat session with an AI agent.`,
	Run: func(cmd *cobra.Command, args []string) {
		if err := runChatSession(); err != nil {
